...
int Extractor::run(ScriptingEnvironment &scripting_environment)
{
    ...
    // by hsb ++: for debug
    CreateNodeShape(coordinates, "../../data/output/node_.shp");
                            ...
                            const unsigned number_of_threads)
{
    ...
    util::Log() << "Input file: " << config.input_path.filename().string(); // osm pdf file
    if (!config.profile_path.empty())
    {
        util::Log() << "Profile: " << config.profile_path.filename().string(); // car.lua
    }
